...
export const SUB_PERSONA_REGISTRY: Record<string, SubPersona> = {
  phd_postdoc: {
    ...
    statisticsDisplay: "Present ALL reported statistics: p-values, confidence intervals, effect sizes, sample sizes, and any performance metrics. Include detailed comparison tables. Explain what each number means in context. Never omit a number — if the paper reports it, include it.",
    ...
    disclaimers: [],
    moduleInstructions: {
      M1: "You are writing for a PhD student or post-doctoral researcher who needs to quickly understand what this paper contributes to the field and whether it's relevant to their own research. Use clear, educational language. Define technical terms on first use. Include a comprehensive metrics table comparing this paper's quantitative results against prior work. Every number matters — sample sizes, effect magnitudes, statistical thresholds, performance gains. Frame the contribution in terms of: What gap does this fill? How does this help me position my own work? What should I cite from this paper? The metrics array should have at least 5 entries covering all reported quantitative outcomes.",
      M2: "You are writing for a PhD student or post-doc who needs to evaluate the claims in this paper to decide whether to build on them. For each claim, extract EVERY reported statistic into the statistics array as individual entries. Build detailed comparison tables where the paper compares against baselines or prior methods. Present effect sizes, confidence intervals, p-values, sample sizes, and any performance benchmarks as separate labeled entries. A claim card with fewer than 3 statistics entries is likely missing data. Explain what each statistic means in context. Highlight which claims are strong enough to build a thesis on vs. which need further validation. Use an encouraging, educational tone.",
