...
export const SUB_PERSONA_REGISTRY: Record<string, SubPersona> = {
  ...
  ai_agent: {
    ...
    label: "AI Agent (API)",
    painPoint: "I need structured, machine-readable data from manuscripts for automated processing.",
    numberPolicy: "all_raw",
    statisticsDisplay: "All raw numbers in structured JSON format. No prose — only data arrays and key-value pairs.",
    inferredDataPolicy: "Never infer. Only report what the paper states. Return null for missing fields.",
    jargonLevel: "assume_domain",
    languageStyle: "Machine-readable. No embellishments, no prose. Use structured keys and arrays.",
    contentGoal: "Provide complete, structured data extraction for downstream machine consumption.",
    depthPreference: "exhaustive",
    comparativeFraming: "Raw data only — no framing or narrative.",
    educationalExtras: false,
    broaderImpactAssessment: false,
    disclaimers: [],
    moduleInstructions: {
      M1: "Return structured JSON with keys: main_contribution (string), novelty_claims (array of strings), quantitative_metrics (array of {metric, value, unit, context}), field_positioning (string), limitations (array of strings). No prose — only structured data.",
      M2: "Return structured JSON with keys for each claim: claim_text (string), evidence_type (string), statistics (array of {test, value, p_value, ci, effect_size, sample_size}), confidence_level (high/medium/low), supporting_figures (array of figure IDs). No prose.",
      M3: "Return structured JSON with keys: steps (array of {step_number, description, parameters, equipment, duration, critical_notes}), reagents (array of {name, concentration, supplier, catalog_number}), equipment (array of {name, model, settings}). No prose.",
      M4: "Return structured JSON with keys: negative_results (array of {hypothesis_tested, outcome, possible_explanations, implications}), limitations (array of {type, description, severity}). No prose.",
  ...
  science_educator: {
    ...
    painPoint: "I need to translate cutting-edge research into engaging, age-appropriate learning materials for my students.",
    numberPolicy: "explained_raw",
    statisticsDisplay: "Key numbers with plain-language interpretations. Include effect sizes explained as everyday comparisons. Explain statistical significance in accessible terms.",
    inferredDataPolicy: "Never infer. Only report what the paper states. If a concept is too advanced, simplify but note the simplification.",
    jargonLevel: "define_all",
    languageStyle: "Educational, scaffolded, encouraging. Use analogies and real-world examples. Build from simple to complex. Age-appropriate adaptations where possible.",
    contentGoal: "Transform research into teachable content: learning objectives, activities, discussion questions, and assessments that bring the science to life in a classroom.",
    depthPreference: "balanced",
    comparativeFraming: "Connect to curriculum standards, prior knowledge students might have, and everyday phenomena.",
    educationalExtras: true,
    broaderImpactAssessment: true,
    disclaimers: ["This educational summary is designed for teaching purposes. Consult the original paper for research-level precision."],
    moduleInstructions: {
      M1: "You are writing for a science educator who needs to understand the paper's contribution to explain it to students. Frame the contribution as: What question were they trying to answer? Why does it matter to society? Use analogies to everyday life. Include 2-3 learning objectives (tagged with Bloom's taxonomy level) that a teacher could use. Provide a 'hook' — an engaging opening question or scenario to start a lesson.",
      M2: "You are writing for a science educator who needs to teach students about evidence and scientific reasoning. For each claim, explain it in plain language with a real-world analogy. Show how the evidence supports the claim — model scientific reasoning. Include discussion questions like 'What would you expect to see if this claim were wrong?' Help students practice evaluating evidence quality.",
      M3: "You are writing for a science educator who wants to design hands-on activities inspired by these methods. Simplify the methodology into steps a student could understand. Suggest classroom-appropriate adaptations or analogous experiments. Identify which steps teach important scientific skills (measurement, controls, observation). Flag safety considerations.",
      M4: "You are writing for a science educator who wants to teach that negative results are valuable in science. Frame each negative result as a learning moment: What did scientists expect? What actually happened? What did they learn? Use this to combat the misconception that 'failed experiments are wasted.' Include a discussion prompt about the value of unexpected results.",
