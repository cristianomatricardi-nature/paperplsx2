...
Deno.serve(async (req) => {
  ...
  const { data: cached } = await supabase
    ...
    .select("content")
    .eq("paper_id", paperId)
    .eq("content_type", "funder_view")
    .eq("persona_id", subPersonaId)
    .is("module_id", null)
    .maybeSingle();

  if (cached) {
    console.log(`[generate-funder-view] Cache hit: paper=${paperId} persona=${subPersonaId}`);
    return new Response(
  ...
  if (missingModules.length > 0) {
    console.log(`[generate-funder-view] Triggering generation for missing modules: ${missingModules.join(", ")}`);
    const generationPromises = missingModules.map(async (moduleId) => {
      try {
        ...
        console.warn(`[generate-funder-view] Module ${moduleId} generation error:`, err);
      }
    });
  ...
  try {
    ...
    if (!aiResponse.ok) {
      ...
      console.error("[generate-funder-view] AI error:", aiResponse.status, errText);
    ...
    if (insertError) {
      console.warn("[generate-funder-view] Cache insert failed (non-blocking):", insertError);
    }
    ...
    console.error("[generate-funder-view] Generation failed:", err);
    return new Response(
      JSON.stringify({
        success: false,
        error: `Funder view generation failed for paper ${paperId}`,
