...
import type { ReplicationCartItem } from './ReplicationCart';

interface AnalyticalPipelineCartProps {
  paperId: number;
...
export function AnalyticalPipelineCart({ paperId, items, onUpdateItems }: AnalyticalPipelineCartProps) {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    try {
      ...
      // ignore
    }}, [items, onUpdateItems]);const removeItem = useCallback((id: string) => {onUpdateItems(items.filter((i) => i.id !== id));}, [items, onUpdateItems]);return <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
    className={cn(
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-foreground">Research comoponents Cart
    ...
    <p className="text-[11px] text-muted-foreground text-center py-3">
          {dragOver ? 'Drop here!' : 'Drag claims, methods, or figures to decompose their analytical pipeline'}
        </p>
    }
    </div>;
